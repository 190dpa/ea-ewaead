...
app.get('/fight', isAuthenticated, async (req, res) => {
    ...
    const { buffs, totalPower, totalHealth } = calculatePlayerBuffs(user.characters || [], user.swords || []);

    // Rework da página de luta
    const content = `
        ...
        <!-- Overlay para Cutscenes de Despertar -->
    ...
    const pageScript = `
            ...
            function playJacketAwakening() {
                ...
                const interval = setInterval(() => {
                    ...
                    if (msgIndex > messages.length) {
                        ...
                        // A cutscene termina, mas os efeitos (música e borda) continuam.
            ...
            function playOverlordAwakening() {
                awakeningOverlay.style.display = 'flex';
                awakeningOverlay.innerHTML = '<div class="overlord-aura"></div>';
                ...
                const interval = setInterval(() => {
                    if (strikeCount >= 20) {
                        clearInterval(interval);
                        awakeningOverlay.classList.remove('awakening-active');
                        awakeningOverlay.style.display = 'none';
                        return;
                    }
            ...
            socket.on('awakening end', () => {
                awakeningOverlay.classList.remove('awakening-active');
                document.body.classList.remove('awakening-active'); // Remove a borda do body
                const audio = document.getElementById('awakening-audio');
                if (audio) {
                    audio.pause();
                    audio.currentTime = 0;
            ...
            function playJacketAbility(targetElement) {
                let hitCount = 0;
                const interval = setInterval(() => {
                    if (hitCount >= 50) { // 50 hits visuais para representar 300
                        clearInterval(interval);
                        return;
                    }
                    const damageEl = document.createElement('div');
                    damageEl.className = 'damage-popup';
                    damageEl.textContent = Math.floor(Math.random() * 10) + 1; // Dano visual aleatório
            ...
            socket.on('battle update', (data) => {
                // Limpa botões de habilidade antigos para evitar duplicatas
                const oldAwakenedBtn = document.getElementById('awakened-ability-btn');
                if (oldAwakenedBtn) {
                    oldAwakenedBtn.remove();
                }
                ...
                if (data.damageToPlayer > 0) showDamage(document.getElementById('player-fighter'), data.damageToPlayer);
                if (data.damageToOpponent > 0) showDamage(document.getElementById('opponent-fighter'), data.damageToOpponent);

                // --- Gatilhos de Efeitos Especiais ---
                if (data.abilityUsed === 'Jacket') {
                    playJacketAbility(document.getElementById('opponent-fighter'));
                }
                if (data.abilityUsed === 'The Overlord') {
                    playOverlordAbility(document.getElementById('opponent-fighter'));
                }
                ...
                if (data.isPlayerTurn) {
                    ...
                    // ✅ Lógica para mostrar botões de habilidade do Despertar
                    if (data.player.awakenedState.active) {
                        document.body.classList.add('awakening-active'); // Adiciona borda RGB na tela
                        ...
                        document.querySelector("[onclick*='use_ability']").style.display = 'none';
                        abilityBtn.style.display = 'none'; // Esconde o botão de Despertar

                        const awakenedBtn = document.createElement('button');
                        awakenedBtn.id = 'awakened-ability-btn';
                        awakenedBtn.className = 'btn btn-special awakening-active'; // Botão também fica RGB
                        awakenedBtn.textContent = data.player.awakenedState.abilityName;
                        awakenedBtn.onclick = () => sendAction('awakened_ability');
                        battleActionsEl.appendChild(awakenedBtn);
                    } else {
                        document.body.classList.remove('awakening-active'); // Garante que a borda saia
                        // Garante que os botões normais voltem a aparecer quando o despertar acabar
                        document.querySelector("[onclick*='fast_attack']").style.display = 'inline-block';
                        document.querySelector("[onclick*='strong_attack']").style.display = 'inline-block';
                        document.querySelector("[onclick*='defend']").style.display = 'inline-block';
                        if(data.canUseAbility) abilityBtn.style.display = 'inline-block';
                    }
                }
            });
...
const charactersByRarity = {
    ...
    SUPREME: [
        { name: 'Jacket', ability: 'Combo Violento', attack: 300, health: 30000, buff: { description: 'Desfere um combo devastador de 300 golpes.', type: 'attack_flat', value: 300 } }
    ]
};
 ...
 */
function calculatePlayerBuffs(characters = [], swords = []) {
    const accumulatedBuffs = {
        attack_percent: 0,
        defense_percent: 0,
        health_flat: 0,
        attack_flat: 0, // ✅ Adicionado para reconhecer o buff do Jacket
    };
    ...
    // Encontra o "personagem ativo" para usar a habilidade (o de maior raridade)
    let activeCharacterForAbility = null;
    if (characters.length > 0) {
        // Clona e ordena os personagens pela raridade (do maior para o menor)
        const sortedCharacters = [...characters].sort((a, b) => {
            const rarityOrder = Object.keys(Rarity); // ['COMUM', 'RARO', ...]
            return rarityOrder.indexOf(b.rarity) - rarityOrder.indexOf(a.rarity);
        });
        activeCharacterForAbility = sortedCharacters[0]; // O primeiro é o mais raro
    }

    for (const dbChar of characters) {
        // ✅ CORREÇÃO: Trata o personagem "The Overlord" como um caso especial
        if (dbChar.name === 'The Overlord') {
            // Se o Overlord está presente, os buffs são absolutos.
            return { buffs: { summary: 'Poder Absoluto. Imune a dano. Dano letal.' }, totalPower: 999999, totalHealth: 9999999, defenseBonus: 1.0, activeCharacter: dbChar };
        }

        // Lógica para os outros personagens
        ...
        if (charTemplate && charTemplate.buff) {
            ...
            // Lógica mais flexível para aplicar buffs
    ...
    if (accumulatedBuffs.attack_flat > 0) descriptions.push(`+${accumulatedBuffs.attack_flat} de Ataque Fixo`);

    // Encontra a melhor espada do jogador (a com maior bônus de ataque)
    ...
    // ✅ Fórmula de cálculo corrigida e aprimorada
    const powerFromPercent = basePower * (1 + accumulatedBuffs.attack_percent);
    const totalPower = Math.floor(powerFromPercent + accumulatedBuffs.attack_flat + bestSword.attackBonus);
    const totalHealth = baseHealth + accumulatedBuffs.health_flat;
...
app.post('/api/admin/generate-gift-link', isAuthenticated, isAdmin, async (req, res) => {
    ...
    if (giftType === 'COINS') {
        ...
        // ✅ CORREÇÃO: Encontra a raridade do personagem para evitar o erro.
    ...
    try {
        ...
        if (giftLink.giftType === 'COINS') {
            ...
            if (charInfo) {
                ...
                const rarityKey = Object.keys(charactersByRarity).find(key => charactersByRarity[key].some(c => c.name === charInfo.name));
...
io.on('connection', (socket) => {
    ...
    socket.on('start battle', async () => {
        ...
        const { totalPower, totalHealth, activeCharacter } = calculatePlayerBuffs(dbUser.characters, dbUser.swords);
